...
def generate_image(
    ...
    sample_list = []
    for class_id in digit_class:
        label = torch.full((batchsize,), class_id).to(device)
    ...
    samples = np.reshape(np.array(sample_list), [nrows * ncols, img_w, img_h])
...
def save_gen_data(
    ...
    with torch.no_grad():
        ...
        bernoulli = torch.distributions.Bernoulli(torch.tensor([p]))
        for i in range(num_samples_per_class // (bs // num_classes)):
            label = torch.LongTensor(digit_class).repeat((bs // len(digit_class)) ).to(device)
            if latent_type == "bernoulli":
                noise = bernoulli.sample((bs, z_dim)).view(bs, z_dim).to(device)
            elif latent_type == "normal":
                noise = torch.randn(bs, z_dim).to(device)
