...
def dp_conv_hook(module, grad_input, grad_output):
    ...
    # # eshipova ver. ====
    # clip_coef = torch.where((grad_input_norm  + 1e-10 < Z), torch.ones_like(clip_coef) * (clip_bound_ / Z), clip_coef)

    # # update Z
    # b = torch.sum(grad_input_norm > (0.7 * Z)).item()
    # b_tilde = (1/batchsize) * (b + noise_multiplier * 10 * torch.randn(1))
    # Z = Z * torch.exp(b_tilde - 0.1)
    # # ===========

    # global ver. ====
    clip_coef = torch.where((grad_input_norm  + 1e-10 < Z), torch.ones_like(clip_coef) * (clip_bound_ / Z), torch.zeros_like(clip_coef))
