...
def main(args):
    ...
    train_ratio_answer = torch.load(ratio_datapath, map_location="cpu")
    ...
    y1_indice = np.where(train_ratio_answer.numpy() == 1)[0]
    y2_indice = np.where(train_ratio_answer.numpy() == 4)[0]
    y_indice = np.concatenate([y1_indice, y2_indice])
    print(len(y1_indice), len(y2_indice), len(y_indice))
    indices_full = y_indice
    ...
    for iter in range(args.iterations + 1):
        ...
        if iter > 0 and iter % args.save_step == 0:
            ...
            save_gen_data(os.path.join(save_dir, 'gen_data_%d.npz' % iter), netGS, z_dim, device0, latent_type=latent_type)
