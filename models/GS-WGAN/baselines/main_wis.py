...
def reweight(loss, ratio):
    loss = loss.view(-1, 1) * torch.pow(ratio.unsqueeze(1), 1.0)

    ratio_sum = ratio.sum()
    loss = loss.sum() / ratio_sum

    return loss

def inf_train_img_y_r(trainloader):
    while True:
        for images, targets, ratio in trainloader:
            yield (images, targets, ratio)
...
def main(args):
    ...
    trainset = data.TensorDataset(train_data, train_label, train_ratio)
    ...
    for i in range(num_discriminators):
        ...
        input_data = inf_train_img_y_r(trainloader)
    ...
    for iter in range(args.iterations + 1):
        ...
        for iter_d in range(critic_iters):
            real_data, real_y, ratio = next(input_data)
            ...
            D_real = reweight(-D_real_score, ratio)
        ...
        label = torch.LongTensor(digit_class)[
            torch.randint(0, 2, [batchsize], device="cpu")
        ].to(device0)
        noisev = autograd.Variable(noise)
